...
function invertCase(str) {
  return str
    ...
    .map((el) => {
      let invert;
      if (el.toLowerCase() === el.toUpperCase()) invert = el;
      if (el.toLowerCase() === el) invert = el.toUpperCase();
      if (el.toUpperCase() === el) invert = el.toLowerCase();
      return invert;
    })
    .join('');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if ((chCode >= 65 && chCode < 78) || (chCode >= 97 && chCode < 110))
      chCode += 13;
    else if ((chCode >= 78 && chCode <= 90) || (chCode >= 110 && chCode <= 122))
      chCode -= 13;
